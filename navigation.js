...
const tierConfig = {
  ...
  async function clickTierAndNavigate(page, tier) {
    ...
    // Scroll down the page to make sure the element is in view
    await page.evaluate(() => {
    ...
    try {
      ...
      if (clicked) {
        ...
        // Wait for any potential navigation or content change
        await Promise.race([
          page.waitForNavigation({ waitUntil: 'networkidle0', timeout: 10000 }).catch(() => {}),
          new Promise(resolve => setTimeout(resolve, 10000))
        ]);
  
        // Check if we've landed on a 404 page
        const currentUrl = await page.url();
